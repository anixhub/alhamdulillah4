...
app.post("/api/upload", async (req, res) => {
  try {
    ...
    const dirsToSave = [
      import_path.default.join(rootDir, "uploads", subFolder),
      import_path.default.join(process.cwd(), "uploads", subFolder),
      import_path.default.join(process.cwd(), "public", "uploads", subFolder),
      import_path.default.join(process.cwd(), "dist", "uploads", subFolder)
    ];
    let currDir = process.cwd();
    for (let i = 0; i < 5; i++) {
      const parentDir = import_path.default.dirname(currDir);
      if (parentDir === currDir) break;
      currDir = parentDir;
      if (import_fs.default.existsSync(import_path.default.join(currDir, "config")) || currDir.includes("hbuilds") || import_fs.default.existsSync(import_path.default.join(currDir, "versions"))) {
        dirsToSave.push(import_path.default.join(currDir, "uploads", subFolder));
      }
    }
    try {
      dirsToSave.push(import_path.default.resolve(process.cwd(), "../../../uploads", subFolder));
      dirsToSave.push(import_path.default.resolve(process.cwd(), "../../../../uploads", subFolder));
    } catch (e) {
    }
...
app.get("/api/uploads/:category/:fileName", (req, res) => {
  try {
    ...
    const rootDir = getPersistentRootDir();
    const possiblePaths = [
      import_path.default.join(rootDir, "uploads", safeCategory, safeFileName),
      import_path.default.join(rootDir, "public", "uploads", safeCategory, safeFileName)
    ];
    let currDir = process.cwd();
    for (let i = 0; i < 5; i++) {
      possiblePaths.push(import_path.default.join(currDir, "uploads", safeCategory, safeFileName));
      possiblePaths.push(import_path.default.join(currDir, "public", "uploads", safeCategory, safeFileName));
      possiblePaths.push(import_path.default.join(currDir, "dist", "uploads", safeCategory, safeFileName));
      const parentDir = import_path.default.dirname(currDir);
      if (parentDir === currDir) break;
      currDir = parentDir;
      possiblePaths.push(import_path.default.join(currDir, "uploads", safeCategory, safeFileName));
    ...
    possiblePaths.push(
      import_path.default.resolve(process.cwd(), "../../../uploads", safeCategory, safeFileName),
      import_path.default.resolve(process.cwd(), "../../../../uploads", safeCategory, safeFileName),
      import_path.default.join(process.cwd(), "uploads", safeCategory, safeFileName),
      import_path.default.join(process.cwd(), "public", "uploads", safeCategory, safeFileName),
      import_path.default.join(process.cwd(), "dist", "uploads", safeCategory, safeFileName)
    );
...
async function startServer() {
  ...
  if (!import_fs2.default.existsSync(publicUploadsPath)) {
    import_fs2.default.mkdirSync(publicUploadsPath, { recursive: true });
  }
  if (!import_fs2.default.existsSync(distUploadsPath)) {
    import_fs2.default.mkdirSync(distUploadsPath, { recursive: true });
  }
  api_default.use("/uploads", import_express2.default.static(publicUploadsPath));
