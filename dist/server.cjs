...
app.post("/api/upload", async (req, res) => {
  try {
    ...
    const rootUploadsDir = import_path.default.join(process.cwd(), "uploads");
    if (!import_fs.default.existsSync(rootUploadsDir)) {
      import_fs.default.mkdirSync(rootUploadsDir, { recursive: true });
    }
    ...
    try {
      import_fs.default.writeFileSync(import_path.default.join(rootUploadsDir, fileName), buffer);
    } catch (e) {
    }
    try {
